...
var WORLD = {
    ...
    setInvalids: function() {
        this.hardStand =
            this.TILES.mountain + this.TILES.forest + this.TILES.water;
    ...
    changeTile: function(x, y, c, checkStructs) {
        ...
        if (
            checkStructs &&
            (item === WORLD.TILES.house ||
                item === WORLD.TILES.city ||
                item === WORLD.TILES.monument)
        ) {
            return;
        }
    ...
    checkWorldNotifsAndLogs: function() {
        ...
        if (
            SETTINGS.notifTraveler === "true" &&
            WORLD.otherPlayers.length > 0 &&
            !WORLD.prevStepHadPlayers
        ) {
            NOTIF.new("traveler nearby", 500);
            ENGINE.log(
                "in the distance, faint against the dark horizon, a traveler appears."
            );
        ...
        if (
            SETTINGS.notifUnknown === "true" &&
            WORLD.otherObjs.length > 0 &&
            !WORLD.prevStepHadObjs
        ) {
        ...
        if (
            SETTINGS.notifUnknown === "true" &&
            !WORLD.prevStepHadMon &&
            boolmon
        ) {
            NOTIF.new("an enormous pillar", 300);
            ENGINE.log(
                "an enormous pillar of rock dominates the sky, its face a deep and smooth purple. it pierces the clouds of ash, splitting them to reveal a small glimpse of the blue sky beyond."
            );
        ...
        if (
            SETTINGS.notifHouse === "true" &&
            !WORLD.prevStepHadHouse &&
            boolhouse
        ) {
        ...
        if (
            SETTINGS.notifCity === "true" &&
            !WORLD.prevStepHadCity &&
            boolcity
        ) {
        ...
        if (booledge && !WORLD.prevStepHadEdge) {
            ENGINE.log(
                "the edge of the world rises out of the distant landscape. the sky gets blotted out as you near it."
            );
        ...
        if (boolproxedge && !WORLD.prevStepNextToEdge) {
            ENGINE.log(
                "a massive wall of smooth gray rock rises into the sky. this does not seem like any natural formation of rock; it juts straight out of the ground at a right angle, and bends up and over your head for many kilometers. if there is a way past this, you clearly are not wanted to go there."
            );
        ...
        if (boolocean && !WORLD.prevStepNextToOcean) {
            ENGINE.log(
                "the calm ocean laps onto the sand at your feet. from this close the acidic moisture hanging over the water can burn your face; this water is not safe for drinking, much less for swimming. clearly, there is no life left here either."
            );
...
!(function(n) {
    ...
    t.simplex2 = function(n, t) {
        ...
        return (
            70 *
            ((A < 0 ? 0 : (A *= A) * A * p.dot2(u, v)) +
                (m < 0 ? 0 : (m *= m) * m * x.dot2(c, y)) +
                (z < 0 ? 0 : (z *= z) * z * q.dot2(M, l)))
        );
    };
})(this);
